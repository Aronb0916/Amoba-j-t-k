...
public class Game {
    ...
    public void updateHighscore(String winnerName, String filename) throws IOException {
        ...
        if (!highscores.has(playerX.getName())) {
            highscores.put(playerX.getName(), 0);
        }
        if (!highscores.has(playerO.getName())) {
            highscores.put(playerO.getName(), 0);
        }
    ...
    public void loadGameFromJson (String s) {
    }
}
